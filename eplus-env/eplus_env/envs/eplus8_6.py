...
from ..util.time import (get_hours_to_now, get_time_string, get_delta_seconds, 
                         get_time_from_seconds, WEEKDAY_ENCODING)
...
class EplusEnv(Env):
    ...
    def __init__(self, eplus_path, weather_path, bcvtb_path, variable_path, idf_path, env_name,
        ...
        self.env_working_dir_parent = self._get_eplus_working_folder(CWD, '-%s-res'%(env_name));
        os.makedirs(self.env_working_dir_parent);
        self.path_name = self.env_working_dir_parent.split("/")[-1]
        self._host = host;
        self._port = port;
        self._socket = s;
    ...
    def reset(self):
        ...
        eplus_working_dir = self._get_eplus_working_folder(
                                self.env_working_dir_parent, '-sub_run');
        ...
        now_weekday, now_hour, now_minute = get_time_from_seconds(curSimTim, YEAR, self._eplus_run_st_mon, 
                          self._eplus_run_st_day, self._eplus_run_st_weekday);
        Dblist.insert(0, now_weekday);
        Dblist.insert(0, now_minute);
        Dblist.insert(0, now_hour);
        ret.append(Dblist);
    ...
    def _create_eplus(self, eplus_path, weather_path, 
        ...
        if platform.system() == 'Windows':
            eplus_exec = os.sep + 'energyplus.exe'
            eplus_process = subprocess.Popen('%s -w %s -d %s %s'
                        %(eplus_path + eplus_exec, weather_path, 
                          out_path, idf_path),
                        shell = True,
